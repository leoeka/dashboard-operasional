...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-8">
        <div>
            <p class="text-sm text-slate-400">Operational overview</p>
        ...
        <a href="{{ route('pages.request') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-800 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-700 transition">
            <i class='bx bx-plus text-lg'></i> New Request
        </a>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        @foreach ([
            ['label' => 'New Requests', 'value' => $requestProjectCount, 'note' => 'Awaiting processing', 'icon' => 'bx-list-plus', 'color' => 'blue'],
            ['label' => 'Total Proposals', 'value' => $totalProposal, 'note' => 'Proposals created', 'icon' => 'bx-file', 'color' => 'amber'],
            ['label' => 'Projects in Progress', 'value' => $activeProjectCount, 'note' => "Out of {$totalProject} total projects", 'icon' => 'bx-loader-circle', 'color' => 'emerald'],
            ['label' => 'Unpaid Invoices', 'value' => $unpaidInvoiceCount, 'note' => 'Rp ' . number_format($unpaidInvoiceAmount, 0, ',', '.'), 'icon' => 'bx-wallet', 'color' => 'rose'],
        ] as $card)
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-5 mb-5">
        <x-card class="xl:col-span-3">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="font-semibold text-slate-800">Project Pipeline</h2>
                    <p class="text-xs text-slate-400 mt-1">Project status breakdown</p>
                </div>
                <a href="{{ route('pages.projects') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-700">View all</a>
            </div>
        ...
        <x-card class="xl:col-span-2">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="font-semibold text-slate-800">Project Types</h2>
                    <p class="text-xs text-slate-400 mt-1">Services currently managed</p>
                </div>
            ...
            <div class="border-t border-slate-100 mt-5 pt-4 flex items-center justify-between">
                <span class="text-xs text-slate-500">Revenue this month</span>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <x-card>
            <div class="flex items-center justify-between mb-5">
                <h2 class="font-semibold text-slate-800">Projects Needing Attention</h2>
                <span class="text-xs text-slate-400">{{ $activeProjectCount }} active</span>
            </div>
            <div class="space-y-3">
                ...
                @empty
                    <p class="text-sm text-slate-400 py-8 text-center">No projects need attention right now.</p>
                @endforelse
            </div>
        </x-card>
        ...
        <x-card>
            <div class="flex items-center justify-between mb-5">
                ...
                <span class="text-xs text-slate-400">Last 6 activities</span>
            </div>
            <div class="divide-y divide-slate-100">
                ...
                @empty
                    <p class="text-sm text-slate-400 py-6 text-center">No activity yet.</p>
                @endforelse
            </div>
        </x-card>
    </div>
