    ...
    <x-card padding="p-4" class="mb-5">
        <form method="GET" class="flex flex-col sm:flex-row gap-3">
            <div class="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2 flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama project atau client..."
                       class="bg-transparent outline-none text-sm w-full text-slate-600">
            </div>
            <select name="status" onchange="this.form.submit()" class="bg-slate-50 text-slate-600 text-sm rounded-lg px-3 py-2 outline-none">
            ...
            <select name="deadline" onchange="this.form.submit()" class="bg-slate-50 text-slate-600 text-sm rounded-lg px-3 py-2 outline-none">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($projects as $project)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-6 py-4"><x-badge :color="$project->statusColor()">{{ $project->statusLabel() }}</x-badge></td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-slate-500">{{ $project->deadline?->translatedFormat('d M Y') ?? '-' }}</td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-3 text-slate-400">
                                <a href="{{ route('pages.projects.show', $project) }}" class="hover:text-brand-500" title="Detail"><i class='bx bx-show text-lg'></i></a>
                                <a href="{{ route('pages.projects.edit', $project) }}" class="hover:text-brand-500" title="Edit"><i class='bx bx-edit text-lg'></i></a>
                                <form method="POST" action="{{ route('pages.projects.destroy', $project) }}" onsubmit="return confirm('Hapus project ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="hover:text-red-500" title="Hapus"><i class='bx bx-trash text-lg'></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="px-6 py-10 text-center text-slate-400">Belum ada project.</td></tr>
                @endforelse
            </tbody>
        </table>
    </x-card>
