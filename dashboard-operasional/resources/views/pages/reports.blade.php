        ...
        <x-slot:actions>
            <div class="flex gap-2">
                ...
                <a href="{{ route('pages.reports.download-excel') }}"
                    class="bg-emerald-600 text-white text-sm font-semibold px-4 py-2.5 rounded-lg flex items-center gap-2 hover:opacity-90 transition">
        ...
        <x-card>
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="bg-emerald-50 rounded-lg p-3">
                    ...
                    <p class="text-lg font-bold text-emerald-700">Rp{{ number_format($paidThisMonth, 0, ',', '.') }}</p>
                </div>
                <div class="bg-amber-50 rounded-lg p-3">
                    <p class="text-xs text-amber-600">Belum Dibayar</p>
                    <p class="text-lg font-bold text-amber-700">Rp{{ number_format($unpaidTotal, 0, ',', '.') }}</p>
                </div>
            </div>
        ...
        <div class="flex items-end gap-4 h-40">
            @foreach ($incomeChart as $item)
                <div class="flex-1 flex flex-col items-center justify-end h-full">
                    ...
                    <p class="text-xs text-slate-400 mt-2">{{ $item['label'] }}</p>
                </div>
            @endforeach
        </div>
    </x-card>
            ...
            <div class="divide-y divide-slate-100">
                @forelse ($topClients as $client)
                    <div class="py-2 flex items-center justify-between">
                        <p class="text-sm text-slate-700">{{ $client->company_name }}</p>
                        <span class="text-xs text-slate-400">{{ $client->projects_count }} project</span>
                    </div>
                @empty
